...
The following code adds a gene expression, a protein abundance, and a methylation layer to ```training```. For illustration purpose we use the same variable selection method `Boruta` and the same learner (`ranger`) for all layers. These  functions fulfill `fuseMLR` requirements in terms of arguments and outputs (see `createTrainLayer` documentation for details). For example, we expect the variable selection function to have at an argument `x` to pass the design matrix of predictors and `y` to pass the response values. In addition, the function must return a vector of selected variables. The use of any method that does not fulfill this criteria -- the Vita variable selection method (implemented by `ranger`) or the Lasso based approach, for instance -- requires either additional wrapping steps or a function to be use to returns a vector the selected variables for a given significant threshold. Also for learners, arguments `x` and `y` are required, and the generic `predict` function must be *callable* for the resulting model. Predictions must be stored either as a vector or a list containing a field `predictions` (a vector of predicted values). If the provided function do not fulfill criteria the argument and output criteria, users should proceed as explained in [E-Interface and wrapping](#e---interface-and-wrapping) to build an interface or wrap their original function for the `fuseMLR` usage.
...
Also add a meta-layer. We use the weighted mean (internal function to `fuseMLR`) as meta-learner. Similarly learners, a meta-learner should allow at least the arguments `x` and `y` to pass the design matrix of predictors and should return a model that allow the call of the generic function `predict` to make predictions for a new dataset (see documentation of `createTrainMetaLayer` for details). If these criteria are not fulfill the explaination in [E-Interface and wrapping](#e---interface-and-wrapping) can be used to map the `x` and `y` to the original argument names or to wrap the original function. We also provide information about possible meta-learner can be used in [Appendix](#appendix).
...
We use the function ```fusemlr()``` to train our models using the subset of selected variables. Here we set `use_var_sel = TRUE` to previously perform variable selection before model training.
...
Three random forests and one weighted meta-model trained on each layer are returned.
...
Prediction performances for layer-specific levels, and on the meta layer are displayed.

- Brier score estimated.

```{r brier, include=TRUE, eval=TRUE}
...
```{r auc, include=TRUE, eval=TRUE, message=FALSE}
pred_values <- predictions$predicted_values
actual_pred <- merge(x = pred_values,
                     y = multi_omics$testing$target,
                     by = "IDS",
                     all.y = TRUE)
y <- actual_pred$disease

# On all patients
perf_auc <- sapply(X = actual_pred[ , 2L:5L], FUN = function (my_pred) {
...
The weighted mean meta-learner is a good aggregation of the layer-specific learners.
...
The interface approach leverages the some arguments in ```createTrainLayer()``` to map the argument names of the original learning function. In the example below, the gene expression layer is re-created using the ```svm``` (Support Vector Machine) function from the ```e1071``` package as the learner. A discrepancy arises in the argument names of the ```predict.svm``` function, which uses ```object``` and ```newdata```.
...
data <- data.frame(
  ...
  Description = c("The weighted mean meta learner. It uses meta data to estimate the weights of the modality-specific models", "The best layer-specific model is used as meta model.", "cobra implements the COBRA (COmBined Regression Alternative), an aggregation method for combining predictions from multiple individual learners")
)
...
- Tay JK, Narasimhan B, Hastie T (2023). Elastic Net Regularization Paths for All Generalized Linear Models. Journal of Statistical Software_, *106*(1), 1-31.

- Kursa M, Rudnicki W.. Feature selection with the Boruta package. J Stat Softw 2010;36:1–13.

- Janitza S, Celik E, Boulesteix A-L.. A computationally fast variable importance test for random forests for high-dimensional data. Adv Data Anal Classif 2016, in press
