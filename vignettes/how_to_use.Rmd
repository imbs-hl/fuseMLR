...
The following code adds a gene expression, a protein abundance, and a methylation layer to ```training```. For illustration purpose we use the same variable selection method `Boruta` and the same learner (`ranger`) for all layers. The corresponding functions can be used with their original arguments and outputs. Otherwise, we would proceed as explained in [E-Interface and wrapping](#e---interface-and-wrapping) to accommodate them to the `fuseMLR` usage.
...
The weighted mean meta-learner combined with mean imputation effectively aggregates the layer-specific learners.
